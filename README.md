# KiRO Software Engineering CoDΞX

<p align="center">
  <a href="https://github.com/Romanap4/kiro-swe-codex/?tab=GPL-3.0-1-ov-file" title="LICENSE"><img src="https://img.shields.io/badge/License-GPL%20v3-blue.svg" alt="An image of a half gray (left side) and half orange (right side) to indicate which license the project is under"></a>
</p>

## How to use it

> [!NOTE]  
> **1** - Clone the repository by running:  

```bash
git clone https://github.com/sdkitagawa/kiro-swe-codex.git
```

> [!TIP]  
> **2** - Choose a Roadmap according to your knowledge level.  
> **3** - Start learning.  

## How to Contribute

> [!IMPORTANT]  
> You can contribute to the project in several different ways:  
> 
> - [**Translate Existing Content**](./CONTRIBUTING.md#translate-existing-content): You can fork this repository, navigate to the `roadmap` folder and then go to the folder containing the content you want to translate and add your translation.  
> - [**Suggest New Content**](./CONTRIBUTING.md##suggest-new-content): If you want, you can suggest that we add new content to the Roadmap by opening issues.  
>   


## Roadmaps

<details>
<summary><b>Junior Roadmap</b></summary>

- ### Fundamentals
    > **Recommendations**  
    > 🚗 **Time Wizard Track**  
    - [How computers work](#)
    - [How programs run in memory](#)
    - [How CPUs work](#)
    - [Operational Systems](#)
    - [Compiled Languages × Interpreted Languages](./roadmap/fundamentals/difference_compiled_vs_interpreted_languages/difference_compiled_vs_interpreted_languages.md)
    - [Strong × Weak Typing and Dynamic × Static Languages](./roadmap/fundamentals/strong_vs_weak_typing_and_dynamic_vs_stactic_languages/strong_vs_weak_typing_and_dynamic_vs_stactic_languages.md)
    - [Linux (Ubuntu or Debian)](#)

<br />

- ### Choose a Programming Language
    > **Recommendations**  
    > 🚗 **Time Wizard Track**  
    - [Python](#)
    - [JavaScript](#)
    - [TypeScript](#)
    > Recommendations  
    > 🚗 **Larvae Moth Track**  
    - [C#](#)
    - [Java](#)
    - [PHP](#)
    - [Ruby](#)
    - [GO](#)
    - [Rust](#)
    - [C++](#)

- ### Learn with Practice
    - [Programming Logic](#)
    - [Algorithms and Data Structures (ALGO/DSA)](#)
    - [Programming Paradigms](#)
        - [Procedural Programming](#)
        - [Functional Programming](#)
        - [Object Oriented Programming (OOP)](#)
            - [Abstraction](#)
            - [Inheritance](#)
            - [Polymorphism](#)
            - [Encapsulation](#)

- ### SQL and Data Base - Learning
    - [CRUD Operations](#)
    - [Data Base Relationships](#)
    - [ACID](#)
    - [Data Base Transactions](#)

- ### SQL and Data Base ➝ Choose a Data Base
    > Recommendations  
    > 🚗 **Time Wizard Track**  
    - [MySQL](#)
    - [SQLite](#)
    > Recommendations  
    > 🚗 **Larvae Moth Track**  
    - [PostgreSQL](#)
    - [MariaDB](#)
    - [Oracle](#)
    - [Microsoft SQL Server (MSSQL)](#)

- ### Devops and Versioning ➝ Choose a Platform
    > 🚗 **Time Wizard Track**  
    - [Git](#)
    - [Git via Terminal](./roadmap/devops/git/how_to_use_git_technology_via_terminal.md)
    > **Platform Recommendations**  
    - [GitHub](#)
    > Recommendations:  
    > 🚗 **Larvae Moth Track**  
    - [SCCS](#)
    - [RCS](#)
    - [CVS](#)
    - [SVN](#)
    - [Git](#)
    - [Diff](#)
    - [SVN](#)
    - [Git](#)
    - [Git via Terminal](./roadmap/devops/git/how_to_use_git_technology_via_terminal.md)
    > **Platform Recommendations**  
    - [GitHub](#)
    - [Gitlab](#)
    - [Bitbucket](#)

- ### Networks and Internet
    - [Web fundamentals](#)
        > 🚗 **Perfect Track**  
        - [How the Internet works](#)
        - [What is a Domain Name System](#)
        - [How DNS works](#)
        - [What a URL is (Structure)](#)
        - [HTTP Protocol](#)
        - [Client × Server Architecture](#)
        - [What a web server is](#)
        - [Cache](#)

- ### Security
    > Recommendations  
    > 🚗 **Time Wizard Track**  
    - [HTTP × HTTPS](./roadmap/security/network_protocols/network_protocols.md#http)
    - [How a login system works](#)
    - [Authentication and authorization](#)
    - [CORS](#)
    - [Cookies](#)
    - [Session](#)
    - [VPN](#)
    - [Firewall](#)
    > Recommendations  
    > 🚗 **Larvae Moth Track**  
    - [Handshake](./roadmap/security/handshake/handshake.md)
    - [HTTP × HTTPS](./roadmap/security/network_protocols/network_protocols.md#http)
    - [FTP](./roadmap/security/network_protocols/network_protocols.md#ftp)
    - [SMTP](./roadmap/security/network_protocols/network_protocols.md#smtp)
    - [POP & IMAP](./roadmap/security/network_protocols/network_protocols.md#pop--imap)
    - [IP](./roadmap/security/network_protocols/network_protocols.md#ip)
    - [TCP](./roadmap/security/network_protocols/network_protocols.md#tcp)
    - [UDP](./roadmap/security/network_protocols/network_protocols.md#udp)
    - [DNS](./roadmap/security/network_protocols/network_protocols.md#dns)
    - [SSH](./roadmap/security/network_protocols/network_protocols.md#ssh)
    - [Google Dorking/Hacking](./roadmap/security/google_dorking_hacking/google_dorking_hacking.md)
    - [How a login system works](#)
    - [Authentication and authorization](#)
    - [CORS](#)
    - [Cookies](#)
    - [Session](#)
    - [VPN](#)
    - [Firewall](#)

- ### API and Integrations
    > Recommendations  
    > 🚗 **Time Wizard Track**  
    - [What is an API](#)
    - [REST APIs and RESTful](#)
    - [JSON - JavaScript Object Notation](#)
    - [HTTP Methods - GET, POST, PUT, DELETE](#)
    - [HATEOAS (Hypermedia as the Engine of Application State)](#)
    - [Pagination, filters and API versioning](#)
    > Recommendations  
    > 🚗 **Larvae Moth Track**  
    - [What is an API](#)
    - [REST APIs and RESTful](#)
    - [JSON - JavaScript Object Notation](#)
    - [HTTP Methods - GET, POST, PUT, DELETE](#)
    - [HATEOAS (Hypermedia as the Engine of Application State)](#)
    - [Pagination, filters and API versioning](#)
    - [gRPC](#)
    - [GraphQL](#)
    - [HTTP3 (Quic)](#)

- ### Model-View-Controller (MVC) ➝ Choose one according to your programming language
    > **Recommendations**  
    > 🚗 **Perfect Track**  
    - [Django - Python](#)
    - [Flask - Python](#)
    - [NestJS - JavaScript and TypeScript](#)
    - [Spring Boot - JavaScript](#)
    - [AdonisJS - JavaScript and TypeScript](#)
    - [Angular - JavaScript and TypeScript](#)
    - [ASP.NET - C#](#)
    - [Symfony - PHP](#)
    - [Laravel - PHP](#)
    - [Ruby on Rails - Ruby](#)
    - [Go - Beego](#)
    - [Go - Revel](#)
    - [Rust - Actix-Web + Diesel](#)
    - [Rust - Rocket](#)
    - [C++ - WebToolkit](#)
    - [C++ - CppCMS](#)

- ### Docker
    > **Recommendations**  
    > 🚗 **Perfect Track**  
    - [Containerization Concepts](#)
    - [Basic Commands](#)
    - [Docker: Images, Volumes, and Networking](#)
    - [Dockerfile](#)
    - [Docker Compose](#)

</details>

<br />

<details>
<summary><b>Mid Roadmap</b></summary>
</details>

<br />

<details>
<summary><b>Senior Roadmap</b></summary>
</details>

<br />

## Contributors

<a href="https://github.com/sdkitagawa/kiro-swe-codex/graphs/contributors"><img src="https://contributors-img.web.app/image?repo=sdkitagawa/kiro-swe-codex&max=500 " alt="List of Contributors" /></a>

<br />

## Authors

| [<img src="https://github.com/sdkitagawa.png?size=50" width=50><br /><sub>@sdkitagawa</sub>](https://github.com/sdkitagawa) | [<img src="https://github.com/Romanap4.png?size=50" width=50><br /><sub>@Romanap4</sub>](https://github.com/Romanap4 ) |
| :---: | :---: |
